...
async function calendarHandler (e) {
    document.querySelector("#selected").innerHTML = 'Valgt dato: ' + e.target.value
    ...
    if (data){
        createMainLayout([
            ...
            newEl({type: 'input', text: 'Click me', attr: {name: 'type', value: 'date'}, event:{name:'change', func: calendarHandler}, class: 'calender'}),
            newEl({type: 'p', text: `ELPRISERNE D. ${day}/${month}-${year}`, attr: {name: 'id', value: 'selected'}}),
            createDataset(data)],
        )
    }
    else {
        createMainLayout([
            ...
            newEl({type: 'input', text: 'Click me', attr: {name: 'type', value: 'date'}, event:{name:'change', func: calendarHandler}, class: 'calender'}),
            newEl({type: 'p', text: `ELPRISERNE D. ${day}/${month}-${year}`, attr: {name: 'id', value: 'selected'}}),
            newEl({type: 'h4', text: 'Der er ingen data for denne dato', class: 'errorText'})],
        )
    }
}
